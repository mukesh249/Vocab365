...
public class Country implements Serializable {
    ...
    public Country(Context context, int num, String name, String countryISO, String code) {
        mNum = num;
        mName = name;
        mCountryISO = countryISO;
        mCountryCode = Integer.parseInt(code);
        mCountryCodeStr = "+" + code;

        String fileName = String.format("f%03d", num);
        mResId = context.getApplicationContext().getResources().getIdentifier(fileName, "drawable", context.getApplicationContext().getPackageName());
    }

    public Country(Context context, String name, String iso) {
        mName = name;
        mCountryISO = iso;
    }
